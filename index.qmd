...
pt <- st_coordinates(here)
res <- om_get(x = c(pt[1], pt[2]), r = 2000)
```
...
# Télécharger, combiner et simplifier les couches géographiques
pt <- st_coordinates(here)
res <- om_get(x = c(pt[1], pt[2]), r = 2000, quiet = TRUE)
...
# Résultat
om_map(res, title = "A 2000 mètres d'ici")
mf_map(st_transform(here, 3857), pch = 22, cex = 2.2, 
       bg = "darkred", add = TRUE)
...
**[osrm](https://github.com/riatelab/osrm)** (Giraud, 2022) permet de produire routes, isochrones et matrices de distances selon différents modes de transport (à pieds, vélo, voiture) à partir d'origines et de destinations
...
![](img/alidade_smooth.png)
[Stadia Aliade Smooth](https://docs.stadiamaps.com/map-styles/alidade-smooth/)
