...
<center>![](img/r_packages_geocoding.png)</center>

## Import de données OSM

<center>![](img/r_packages_extract_objects.png)</center>

## Import tuiles raster et couches d'habillage carto

<center>![](img/r_packages_map_templates_static.png)</center>

## Tuiles raster pour carto interactive

<center>![](img/r_packages_map_templates_interactive.png)</center>

## Calcul d'itinéraires

<center>![](img/r_packages_map_routing.png)</center>

## Maintenus par le RIATE

<center>![](img/r_packages_riate.png)</center>


## Quel restaurant Thai dans le secteur ? 

**[tidygeocoder](https://jessecambon.github.io/tidygeocoder/)** (Cambon, 2025) : Enrichir des données à partir de services de géocodage qui utilise par défaut l'API [Nominatim](https://nominatim.org/) qui utilise les adresses OpenStreetMap. D'autres services sont aussi disponibles (Geocodio, Mapbox, HERE, TomTom, etc.)
...
**[osmdata](https://github.com/ropensci/osmdata)** (Padgham, 2023) permet d'importer des objets vectoriels d'OSM à partir du serveur [overpass](https://overpass-api.de/). Pour de grosses requêtes, préférez le package [osmextract](https://github.com/ropensci/osmextract) (Gilardi, 2025).
...
**[osrm](https://github.com/riatelab/osrm)** (Giraud, 2024) permet l'interface entre R et le service de routage [OSRM](https://project-osrm.org/). Ce package permet de créer des routes, isochrones et matrices de distance à pieds/vélo/voiture. Pour de large requêtes, créer [sa propre instance OSRM](https://rcarto.github.io/posts/build_osrm_server/) pour ne pas surcharger le serveur de démo.
...
om_map(res, title = "A 2000 mètres d'ici")
mf_map(st_transform(routes, 3857), add = TRUE)
mf_map(st_transform(here, 3857), add = TRUE)
mf_map(st_transform(thai, 3857), pch = 21, bg = "red",  add = TRUE)
...
## C'est un peu loin, il faut le reconnaitre

[leaflet](https://rstudio.github.io/leaflet/) (Cheng, 2024) permet de créer des cartes interactives en utilisant la librairie JavaScript Leaflet et le package htmlwidgets. Plusieurs [tuiles de référence](https://leaflet-extras.github.io/leaflet-providers/preview/index.html) sont utilisables (OpenStreetMap = défaut).
...
Cas d'usage du RIATE (2017-2025)

- [Typologie socio-économique des régions de l'Union Européenne](https://hal.science/hal-03610513) (CGET, 2016)
- [Apport des données OpenStreetMap pour l'analyse des centralités commerciales dans les espaces transfrontaliers](https://hal.science/hal-03587399/file/RIATE_Rapport_OSM_Octobre2020.pdf) (CGET, 2020)
- [GRANULAR]() (Horizon Europe 2024 ->)
- Quelques explorations plus "personnelles" : accès aux salles d'escalade parisiennes



## Une typologie des territoires frontaliers (CGET, 2016)

Complémentarité des territoires frontaliers européens à partir d'une typologie basée sur des données socio-économiques "classiques" (source : Eurostat) 
...
Création d'indicateurs pour qualifier l'**accessibilité** des des territoires frontaliers à partir d'OSM : **temps de parcours moyens entre deux régions frontalières**.
...
<small>Pour en savoir plus, accès au rapport final, la présentation de synthèse et une collection de rapports techniques à partir de cette archive [HAL](https://hal.science/hal-03610513)</small>


## Identifier des centralités frontalières avec OSM ? (CGET, 2020)

Sélection de 15 équipements marqueurs de centralité disponibles *a priori* dans OpenStreetMap. 

![](img/cget20_01.png)

## Identifier des centralités frontalières avec OSM ? (CGET, 2020)

Comparaison française avec la Base Permanente des équipements pour évaluer le niveau de **complétude d'OSM** pour ces points d'intérêt.
...
Evaluation de **sur/sous représentations de POI** toute chose égale par ailleurs au regard de la population. 
...
## Qualifier l'accessibilité des territoires ruraux européens (Horizon Europe, 2024)
