...
- Présentation d'OpenStreetMap : spécificités, histoire, organisation, usages
...
- Collectivités (voir [Recensement (2018)](https://labo.societenumerique.gouv.fr/fr/articles/ces-collectivit%C3%A9s-qui-misent-sur-openstreetmap-et-la-cartographie-participative/) d'initiatives menées par des collectivités ou une [enquête en  2020](https://labo.societenumerique.gouv.fr/fr/articles/lutilisation-et-la-contribution-%C3%A0-openstreetmap-par-lacteur-public/) sur la vision d'OSM par l'acteur public. )
...
**Corrections topologiques préalable à l'utilisation du réseau routier OSM**
...
**Part des objets de type buildings ayant pour attribut

![](img/attribute_completeness_1.jpg)
::::

:::: {.column width="50%" .small}

**Part des objets buildings ayant pour attribut un tag découragé par la communauté**
...
L'outil [Ohsome](https://dashboard.ohsome.org) développé par l'[HeiGIT](https://heigit.org/) propose **plusieurs métriques** qualifier la qualité d'OSM à travers le temps et l'espace.
...
Comparaison à des jeux de données de référence (Microsoft Buildings, EURO) : surface couverte. 
...
Comparaison à des jeux de données de référence (Microsoft Roads) : longeur des tronçons 
...
Évaluation de la saturation des contributions. Elle est élevée si la croissance de la courbe ajustée est minimale. 
...
**[tidygeocoder](https://jessecambon.github.io/tidygeocoder/)** (Cambon, 2025) : permet d'enrichir des données à partir de services de géocodage (Geocodio, Mapbox, HERE, TomTom, etc.) dont l'API [Nominatim](https://nominatim.org/) qui utilise les adresses OpenStreetMap
...
- Mise en place d'un serveur de routage Europe entière (package [`osrm`](https://github.com/riatelab/osrm)), parallélisation ([`parallel`](https://r-universe.dev/manuals/parallel.html)), optimisation de workflows ([`targets`](https://books.ropensci.org/targets/))
...
:::: {.medium}
<center>**En savoir plus ?** Dépôt [Zenodo](https://zenodo.org/records/14160729) (données et méthodes), outil de [géovisualisation](https://nlambert.gitpages.huma-num.fr/granular-gridviz/)</center>
::::
...
éléments mis de côté de peur d'exploser le temps mais qui peuvent être rajoutés si cela vous semble nécessaire ou pertinent. La plupart du temps il seront évoqués à l'oral avec renvoi vers des ressources de référence. 

- présentation des logiciels hors OSM qui utilisent / diffusent des données OSM
- ressources RIATE qui présentent les méthodes type KDE. 
- comment installer un serveur osrm

Ce qui pourrait être mis de côté

- Usages OSM (bof)

Des questions

- Pas too much au niveau de la prez des projets ? 